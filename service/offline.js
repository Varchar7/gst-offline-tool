...
const { Validator } = require('../node_modules/jsonschema');
var getSchema = require('../public/uiassets/json/getSchema.json')
...
function saveMstrforprod(req, res, err) {
    ...
    try {
        ...
        fileData = JSON.parse(fileData);
         if(append){
           var pMaster = (rectype == "productsMasters") ? fileData.productsMasters.concat(mstrData): fileData.productsMasters;
             if(rectype == 'supplierRecipientMasters' && mstrData.length>0){
             for(let i=0; i<fileData.supplierRecipientMasters.length;i++){
              if(fileData.supplierRecipientMasters[i].gstin == mstrData[0].gstin){
                    res.send({error:"gstinerror"});
                    return;
                   }
                 if(fileData.supplierRecipientMasters[i].supplierRecipientName.toLowerCase() == mstrData[0].supplierRecipientName.toLowerCase()){
                 
                  res.send({error:"superror"});
                  return;
                 }
             }}
            
             var sMaster = (rectype == "supplierRecipientMasters") ? fileData.supplierRecipientMasters.concat(mstrData) : fileData.supplierRecipientMasters; 
...
function getMasterData(req, res) {
    ...
    logger.log("error", "GET MSTR NO JSON FILE EXISTS FOR SELECTION");
        res.end();
    }
}
...
function listJsonData(req, res) {
    ...
    try {
       ...
       logger.log("info", "Data NO JSON FILE EXISTS FOR SELECTION");
       res.status(404).end();
    }
...
var importFile = function (req, res) {
    ...
    try {
        ...
        var mstrfile = JSON.parse(content);
        if (mstrfile.productsMasters && mstrfile.productsMasters.length > 50) {
            //  var countrec = {errorMsg : "Exceeding length",
            //  count:mstrfile.productsMasters.length};
            res.send("Exceeding length");
            return;

        }
        if (mstrfile.supplierRecipientMasters && mstrfile.supplierRecipientMasters.length > 50) {
            //  var countrec = {errorMsg : "Exceeding length",
            //  count:mstrfile.productsMasters.length};
            res.send("SupExceeding length");
            return;

        }

        var validator = new Validator();
        var result = validator.validate(mstrfile, getSchema, { nestedErrors: true });
         if (result.errors.length > 0) {
            ...
            if (mstrfile.productsMasters) {

                for (let i = 0; i < mstrfile.productsMasters.length; i++) {
                    for (let j = 0; j < mstrfile.productsMasters.length; j++) {
                        if (i != j && mstrfile.productsMasters[i].productName == mstrfile.productsMasters[j].productName) {
                            res.send("ProductName duplicate");
                            return;
                        }
                    }
                }
            }
            if (mstrfile.supplierRecipientMasters) {

                for (let i = 0; i < mstrfile.supplierRecipientMasters.length; i++) {
                    for (let j = 0; j < mstrfile.supplierRecipientMasters.length; j++) {
                        if (i != j && mstrfile.supplierRecipientMasters[i].gstin == mstrfile.supplierRecipientMasters[j].gstin) {
                            res.send("gstin duplicate");
                            return;
                        }
                        if (i != j && mstrfile.supplierRecipientMasters[i].supplierRecipientName == mstrfile.supplierRecipientMasters[j].supplierRecipientName) {
                            res.send("supplierRecipientName duplicate");
                            return;
                        }
                    }
                }
            ...
            if (mstrfile.productsMasters) {
                for (let i in mstrfile.productsMasters) {
                    mstrfile.productsMasters[i].actionFlag = "U";
                }
            }
            if (mstrfile.supplierRecipientMasters) {
                for (let i in mstrfile.supplierRecipientMasters) {
                    mstrfile.supplierRecipientMasters[i].actionFlag = "U";
                }
            }
            try {
                if (fs.existsSync(__dirname + "/../public/userData/master_" + mstrfile.userGstin + ".json")) {
                    let fileData = fs.readFileSync(__dirname + "/../public/userData/master_" + mstrfile.userGstin + ".json")
                    let addedData = JSON.parse(fileData);
                    if (addedData.productsMasters && addedData.productsMasters.length > 0) {
                        let tempData = [];
                        for(let i in addedData.productsMasters){
                            if(addedData.productsMasters[i].actionFlag == null  || addedData.productsMasters[i].actionFlag == ""){
                                tempData.push(addedData.productsMasters[i]);
                            }
                        }
                        addedData.productsMasters = tempData.concat(mstrfile.productsMasters);
                    }
                    else {
                        addedData.productsMasters = mstrfile.productsMasters;
                    }
                    if (addedData.supplierRecipientMasters && addedData.supplierRecipientMasters.length > 0) {
                        let tempData = [];
                        for(let i in addedData.supplierRecipientMasters){
                            if(addedData.supplierRecipientMasters[i].actionFlag == null  || addedData.supplierRecipientMasters[i].actionFlag == ""){
                                tempData.push(addedData.supplierRecipientMasters[i]);
                            }
                        }
                        addedData.supplierRecipientMasters = tempData.concat(mstrfile.supplierRecipientMasters);
                    }
                    else {
                        addedData.supplierRecipientMasters = mstrfile.supplierRecipientMasters;
                    }
                    mstrfile = addedData;
                }
            } catch (err) {
                ...
                fs.writeFileSync(__dirname + "/../public/userData/master_" + mstrfile.userGstin + ".json", JSON.stringify(mstrfile));
                res.send(mstrfile);
            }
